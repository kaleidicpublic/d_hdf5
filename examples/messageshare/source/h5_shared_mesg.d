...
string f_default="default_file.h5";
/*-------------------------------------------------------------------------
 * Function:    main
 *
...
int main(string[] args)
{
     ...
     */
    create_standard_file(f_default, fcpl_id);
    if(ret < 0) goto error;
    ...
    H5P.set_shared_mesg_nindexes(fcpl_id, 1);
    if(ret < 0) goto error;
     ...
     */
    H5P.set_shared_mesg_index(fcpl_id, 0, H5O_SHMESG_ALL_FLAG, 40);
    if(ret < 0) goto error;
     ...
     */
    H5P.set_shared_mesg_phase_change(fcpl_id, 30, 20);
    if(ret < 0) goto error;
     ...
     * written later.
     */
    create_standard_file("one_index_file.h5", fcpl_id);
    if(ret < 0) goto error;
     ...
     * overhead).
     */
    H5P.set_shared_mesg_index(fcpl_id, 0, H5O_SHMESG_ALL_FLAG, 1000);
    if(ret < 0) goto error;

    create_standard_file("only_huge_mesgs_file.h5", fcpl_id);
    if(ret < 0) goto error;
     ...
     */
    H5P.set_shared_mesg_index(fcpl_id, 0, H5O_SHMESG_SDSPACE_FLAG | H5O_SHMESG_ATTR_FLAG, 40);
    if(ret < 0) goto error;

    create_standard_file("only_dspaces_and_attrs_file.h5", fcpl_id);
    if(ret < 0) goto error;
     ...
     */
    H5P.set_shared_mesg_nindexes(fcpl_id, 2);
    if(ret < 0) goto error;
    H5P.set_shared_mesg_index(fcpl_id, 0, H5O_SHMESG_DTYPE_FLAG | H5O_SHMESG_SDSPACE_FLAG, 40);
    if(ret < 0) goto error;
    H5P.set_shared_mesg_index(fcpl_id, 1, H5O_SHMESG_ATTR_FLAG, 40);
    if(ret < 0) goto error;

    create_standard_file("separate_indexes_file.h5", fcpl_id);
    if(ret < 0) goto error;
    ...
    H5P.set_shared_mesg_phase_change(fcpl_id, 5, 0);
    if(ret < 0) goto error;

    create_standard_file("small_lists_file.h5", fcpl_id);
    if(ret < 0) goto error;
     ...
     */
    H5P.set_shared_mesg_phase_change(fcpl_id, 0, 0);
    if(ret < 0) goto error;

    create_standard_file("btrees_file.h5", fcpl_id);
    if(ret < 0) goto error;
 ...
 *-------------------------------------------------------------------------
 */
herr_t create_standard_file(string filename, hid_t fcpl_id)
{
    hid_t file_id=-1;
    ...
    /* Create the file */
    file_id = H5F.create(filename, H5F_ACC_TRUNC, fcpl_id, H5P_DEFAULT);
     ...
     */
    temp_type_id = H5T.array_create2(H5T_NATIVE_INT, 2, dims);
    if(temp_type_id < 0) goto error;
    type_id  = H5T.array_create2(temp_type_id, 2, dims);
    if(type_id < 0) goto error;
    H5T.close(temp_type_id);
     ...
     * 10 integers.
     */
    attr_type_id = H5T.copy(H5T_NATIVE_INT);
    if(attr_type_id < 0) goto error;
    attr_space_id = H5S.create_simple(dims);
     ...
     */
    foreach(x;0.. NUM_DATASETS)
    {
       dset_id = H5D.create2(file_id, DSETNAME[x], type_id, space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
       if(dset_id < 0) goto error;

       /* Create an attribute on the dataset */
       attr_id = H5A.create2(dset_id, "attr_name", attr_type_id, attr_space_id, H5P_DEFAULT, H5P_DEFAULT);
       if(attr_id < 0) goto error;

       /* Write data to the attribute */
       H5A.write(attr_id, H5T_NATIVE_INT, cast(ubyte*)attr_data);
       H5A.close(attr_id);
       H5D.close(dset_id);
    }

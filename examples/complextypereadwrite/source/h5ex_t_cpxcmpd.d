...
{
    writefln("h5ex_t_cpxcmpd not yet ported");
}
/+
enum filename="h5ex_t_cpxcmpd.h5";
enum DATASET         ="DS1";
enum DIM0            =2;
...
{
    hid_t       vehicletype, colortype, sensortype, sensorstype, loctype,
     ...
     */
    auto file = H5F.create (filename, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
    ...
    space = H5S.create_simple (adims2);
    dset = H5D.create2 (file, "Ambient_Temperature", H5T_NATIVE_DOUBLE, space,
     ...
     */
    group = H5G.create2 (file, "Land_Vehicles", H5P_DEFAULT,H5P_DEFAULT,H5P_DEFAULT);
     H5G.close (group);
    group = H5G.create2 (file, "Air_Vehicles", H5P_DEFAULT,H5P_DEFAULT,H5P_DEFAULT);
     H5G.close (group);
    ...
    ptr[0].location = cast(char*)"Exterior (static)".toStringz;
    ptr[0].temperature = 53.23;
    ptr[0].pressure = 24.57;
    ptr[1].serial_no = 1184;
    ptr[1].location = cast(char*)"Intake".toStringz;
    ptr[1].temperature = 55.12;
    ptr[1].pressure = 22.95;
    ptr[2].serial_no = 1027;
    ptr[2].location = cast(char*)"Intake manifold".toStringz;
    ptr[2].temperature = 103.55;
    ptr[2].pressure = 31.23;
    ptr[3].serial_no = 1313;
    ptr[3].location = cast(char*)"Exhaust manifold".toStringz;
     ...
     */
    wdata[0].name = cast(char*)"Airplane".toStringz;
    wdata[0].color = GREEN;
    ...
    ptr = cast(sensor_t *) malloc (wdata[1].sensors.len * sizeof (sensor_t));

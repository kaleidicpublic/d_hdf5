...
immutable float[ROWS*COLS] windchillF =
...
void PRINT(float[ROWS*COLS] array)
{
    foreach(i;0..ROWS)
    {
	   foreach(j;0..COLS)
	       writef("%6.2f ", array[i*COLS+j]);
	   writefln("");
    }
}
...
int main(string[] args)
{
    ...
    H5Dwrite(dataset, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, H5P_DEFAULT, cast(ubyte*)windchillF.ptr);
    ...
    H5Dread(dataset, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, dxpl_id_f_to_c, cast(ubyte*)windchillC.ptr);
    ...
    H5Dwrite(dataset, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL,dxpl_id_f_to_c, cast(ubyte*)windchillF.ptr);
    ...
    status = H5Dwrite(dataset, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, dxpl_id_f_to_c, cast(ubyte*)windchillF.ptr);
    writefln("* Read the data with the c_to_f data transform");
    H5Dread(dataset, H5T_NATIVE_FLOAT, H5S_ALL, H5S_ALL, dxpl_id_c_to_f, cast(ubyte*)windchillC.ptr);
    ...
    writefln("************** PART 4 **************");
    auto transform_size = H5Pget_data_transform(dxpl_id_f_to_c, null, 0);
    transform.length=transform_size+1;
    H5P.get_data_transform(dxpl_id_f_to_c,transform);

    writefln("\nTransform string (from dxpl_id_f_to_c) is: %s", cast(string)(cast(char*)transform).fromStringz);
